...
const (
	...
	ExternalAuthError    Type = "EXTERNAL_AUTH_ERROR"    // External authentication service errors
)
...
// NewExternalAuthError creates an error for external authentication failures
func NewExternalAuthError(service string, details string) *Error {
	return &Error{
		Type:    ExternalAuthError,
		Message: fmt.Sprintf("%s authentication error: %s", service, details),
	}
}
